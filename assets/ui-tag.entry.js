...
const Tag = class {
    ...
    label;
    /**
     * Underlying value for the tag (useful for IDs)
     */
    value;
    /**
     * Color variant
    ...
    icon;
    /**
     * Orientation of the tag: Orientation | 'vertical-text' | 'vertical-rotated'
     */
    orientation = 'horizontal';
    /**
     * Shape of the tag: Shape | 'ribbon' | 'ticket' | 'coupon'
     */
    shape = 'rounded';
    /**
     * Whether to make the icon spin (useful for loading/syncing)
     */
    iconSpin = false;
    /**
     * Custom close icon (replacing the default Ionicon 'close')
     */
    closeIcon;
    /**
    ...
    customColor;
    /**
     * Custom text color hex string
     */
    customTextColor;
    /**
     * Tooltip text override for hover
     */
    tooltip;
    /**
    ...
    disabled = false;
    /**
     * Explicit icon position (unifying names)
     */
    iconPosition = 'left';
    /**
     * Icon library (unifying with other components)
     */
    iconLibrary = 'default';
    /**
    ...
    userAvatar;
    /**
     * Whether the tag is checkable (selectable)
     */
    checkable = false;
    /**
    ...
    /**
     * Whether the tag is clickable (shows pointer and hover effect)
     */
    clickable = false;
    /**
     * Elevation level (0-3)
    ...
    loading = false;
    /**
     * Whether to show a pulse animation (for attention)
     */
    pulse = false;
    /**
     * Whether to generate color automatically based on label hash
     */
    autoColor = false;
    /**
     * Gradient variant (e.g., 'blue-purple', 'green-teal')
     */
    gradient;
    /**
    ...
    maxWidth;
    /**
     * Size of the icon (e.g., '14px', '1em')
     */
    iconSize = '1.14em';
    /**
     * Whether to show as a skeleton (placeholder)
     */
    skeleton = false;
    /**
     * Show tooltip on hover (useful with maxWidth)
     */
    showTooltip = false;
    /**
    ...
    handleClick = () => {
        ...
        if (this.checkable) {
            ...
            this.tagCheck.emit(this.checked);
        }
    };
    componentDidLoad() {
        setTimeout(() => {
    ...
    render() {
        if (!this.visible)
            return null;
        const autoColorData = this.autoColor ? this.getColorFromHash(this.label ?? '') : null;
        const classes = {
            'tag': true,
            [`tag-${this.color}`]: !this.autoColor && !this.gradient,
            [`tag-${this.size}`]: true,
            [`tag-${this.variant}`]: true,
            [`tag-shape-${this.shape}`]: true,
            'tag-rounded': this.shape === 'rounded', // Legacy compatibility
            'tag-checkable': this.checkable,
            'tag-clickable': this.clickable,
            'tag-checked': this.checked,
            'tag-loading': this.loading,
            'tag-disabled': this.disabled,
            'tag-pulse': this.pulse,
            'tag-skeleton': this.skeleton,
            [`tag-${this.orientation}`]: this.orientation !== 'horizontal',
            [`tag-elevated-${this.elevation}`]: this.elevation > 0,
            [`tag-closing-${this.animation}`]: this.isClosing,
            [`tag-opening-${this.animation}`]: this.isOpening,
            [`tag-gradient-${this.gradient}`]: !!this.gradient && !this.gradient.includes('gradient'),
            'tag-custom-gradient': !!this.gradient && this.gradient.includes('gradient'),
            'tag-truncate': !!this.maxWidth,
        };
        const style = {
            '--animation-duration': `${this.animationDuration}ms`,
        };
        if (this.gradient && this.gradient.includes('gradient')) {
            style['background'] = this.gradient;
            style['border'] = 'none';
            style['color'] = '#fff'; // Default to white for gradients
        }
        if (this.maxWidth) {
        ...
        if (autoColorData) {
            style['background-color'] = autoColorData.bg;
            style['color'] = 'white';
            style['border-color'] = 'transparent';
            style['--tag-glow-color'] = autoColorData.rgb;
        }
        const titleText = this.tooltip || (this.showTooltip ? this.label : undefined);
        const isInteractive = this.clickable || this.checkable || this.removable;
        return (h("span", { class: classes, onClick: this.handleClick, onKeyDown: this.handleKeyDown, style: style, title: titleText, dir: this.rtl ? 'rtl' : 'ltr', tabIndex: isInteractive && !this.disabled ? 0 : -1, role: isInteractive ? 'button' : undefined, "aria-pressed": this.checkable ? this.checked : undefined, "aria-disabled": this.disabled ? 'true' : undefined }, this.skeleton ? (h("span", { class: "tag-skeleton-content" })) : [
            this.variant === 'dot' && h("span", { class: "tag-dot-indicator" }),
            this.iconPosition === 'left' && this.renderMedia(),
            h("span", { class: "tag-label" }, this.loading && !this.label ? 'Loading...' : this.label),
            (this.shape === 'ticket' || this.shape === 'coupon') && h("span", { class: "tag-ticket-hole tag-hole-right" }),
            this.shape === 'coupon' && h("span", { class: "tag-ticket-hole tag-hole-left" }),
            this.iconPosition === 'right' && this.renderMedia(),
            this.badge && (h("span", { class: `tag-badge tag-badge-${this.badgeColor}` }, this.badge)),
            this.counter && (h("span", { class: "tag-counter" }, this.counter)),
            this.removable && !this.disabled && (h("ui-button", { variant: "ghost", size: "xxs", iconOnly: true, class: "tag-remove", onClick: this.handleRemove, ariaLabel: "Remove tag", tabIndex: -1, icon: this.closeIcon || "x", iconLibrary: "lucide", iconSize: "14px" }))
        ]));
    }
    renderMedia() {
        if (this.loading && this.variant !== 'dot') {
            return h("ui-loader", { type: "spinner", size: "0.8em", color: "inherit" });
        }
        if (this.userAvatar)
            return h("img", { src: this.userAvatar, alt: "", class: "tag-avatar" });
        if (this.image)
            return h("img", { src: this.image, alt: "", class: "tag-image" });
        if (this.icon)
            return (h("span", { class: { 'tag-icon': true, 'tag-icon-spin': this.iconSpin } }, h("ui-icon", { name: this.icon, library: this.iconLibrary, size: this.iconSize })));
        return null;
    }
};
Tag.style = tagCss();

export { Tag as ui_tag };
